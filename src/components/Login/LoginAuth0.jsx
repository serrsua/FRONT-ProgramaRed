  ...
  useEffect(() => {
    async function fetchData() {
      if (isAuthenticated) {
        ...
        if (responseUserCreate.data.created) {
          //si es nuevo usuario
          try {
            if (!data[0].email)
              throw new Error(
                "Te logueaste correctamente, sin embargo no se pudo enviar el correo de bienvenida, registra tu email"
              );
