...
export default function DashboardAdmin() {
  ...
  const countReports = async () => {
    try {
      ...
      if (res.status === 200) {
        ...
        setCountedReports({
          reportedUsers: res.data.reportedUsers.map((ru) => ({
            username: ru.username,
            cantidad: Number(ru.cantidad),
          })),
          reportedPosts: res.data.reportedPosts.map((rp) => ({
            post: rp.post,
            cantidad: Number(rp.cantidad),
          })),
          reportedComments: res.data.reportedComments.map((rc) => ({
            comment: rc.comentario,
            cantidad: Number(rc.cantidad),
          })),
        });
      }
    } catch (error) {
  ...
  return (
    <div className="m-auto text-center">
      <NavLink className="flex justify-center w-full " to="/home">
        <h2 className=" self-center text-lg font-bold py-2 px-3 bg-mediumGreen text-gray-800 mt-4 hover:text-white hover:bg-slate-900 rounded-md">
          Volver
        </h2>
      </NavLink>
