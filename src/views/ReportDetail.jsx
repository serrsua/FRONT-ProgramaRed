  ...
  return (
    <div className="DIV_APP flex justify-start flex-col items-start h-full overflow-hidden w-full relative">
      <h3 className="text-3xl mt-5">
        Reporte de {reportDetail?.User.username}
      </h3>
      <span className="text-2xl mt-5">
        Creado el {new Date(reportDetail?.createdAt).toLocaleString("es-AR")}
      </span>
      ...
      <p className="text-lg mt-2 mb-5">
        {reportDetail?.username ? (
          ...
          <>
            <span className="text-2xl font-medium">Comentario Reportado:</span>
      ...
      <Link
        to={"/admin"}
        className="bg-green-600 rounded-lg px-5 py-3 w-full text-center font-medium hover:bg-green-400 transition ease-out delay-75"
      >
        Volver
      </Link>
    </div>
